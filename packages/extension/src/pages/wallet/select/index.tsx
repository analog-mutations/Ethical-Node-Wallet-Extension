...
import { OpenMenu } from "../../../components/icon/openMenu";
import { CloseMenu } from "../../../components/icon/closeMenu";
...
const KeyringItem: FunctionComponent<{
  ...
  const paragraph = useMemo(() => {
    if (keyInfo.insensitive["bip44Path"]) {
      ...
      return `m/44'/${coinType >= 0 ? coinType : "-"}'/${bip44Path.account}'/${bip44Path.change
  ...
  return (
    <>
      <Box
        padding="1rem"
        minHeight={isMenuOpen ? "7rem" : "4.625rem"}
        backgroundColor={
          theme.mode === "light"
          ...
          <Box style={{
          ...
          }}>
            {
              dropdownItems.map((item) => {
                return (
                  <XAxis alignY="center" key={item.key}>
                    <Button
                      text={item.label}
                      size="extraSmall"
                      color="secondary"
