...
export interface ProxyRequestResponse {
  type: "proxy-request-en-response";
  id: string;
...
export function injectKeplrToWindow(keplr: IKeplr): void {
  defineUnwritablePropertyIfPossible(window, "ethicalNode", keplr);
  ...
  static startProxy(
    ...
    eventListener.addMessageListener(async (e: any) => {
      const message: ProxyRequest = parseMessage
        ? parseMessage(e.data)
        : e.data;
      if (!message || message.type !== "proxy-request-en") {
        return;
      }
  ...
  protected requestMethod(
    ...
    return new Promise((resolve, reject) => {
      const receiveResponse = (e: any) => {
        ...
        if (
          !proxyResponse ||
          proxyResponse.type !== "proxy-request-en-response"
        ) {
          return;
        }
